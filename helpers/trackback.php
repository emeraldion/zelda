...
/**
 * PHP Class to handle TrackBacks (send/ping, receive, retrieve, detect, seed, etc...)
 *
 * <code><?php
...
class Trackback {
    ...
    var $get_id = ''; // retrieves and holds $_GET['id'] (if not empty)
    var $post_id = ''; // retrieves and holds $_POST['id'] (if not empty)
    var $url = ''; // retrieves and holds $_POST['url'] (if not empty)
    var $title = ''; // retrieves and holds $_POST['title'] (if not empty)
    var $excerpt = ''; // retrieves and holds $_POST['excerpt'] (if not empty)
    /**
     * Class Constructure
     *
     * @param string $blog_name
    ...
    function fetch($success = false, $response = "")
    {
        if (!$success && empty($response)) {
            $response = "An error occured while tring to retrieve trackback information...";
        }
        ...
        // Send back response...
        if ($success) {
            // Trackback retrieved successfully...
    ...
    /**
     * Cuts a string short (with "...") according to $max_length...
     *
     * @param string $string
...
function technorati_ping($title, $url, $link)
{
	/**	
	*	Technorati API
	*	--------------
	...
	*/
	...
	$tb = "http://rpc.technorati.com/rpc/ping";
	$response = "";
	$reason = "";
	// Set default values
	if (empty($title)) {
		$title = "Emeraldion Lodge";
	}
	$target = parse_url($tb);
	...
	$tb_send = "<" . '?xml version="1.0"?' . ">\r\n" .			   
				...
				"  </params>\r\n" .
				"</methodCall>";
	$tb_headers = 	"POST " . $target["path"] . $target["query"] . " HTTP/1.0\r\n" .
					"User-Agent: EmePavilion/2.1 (Atom 0.3; Emeraldion)\r\n" .
					"Host: " . $target["host"] . "\r\n" .
					"Content-type: text/xml\r\n" .
					"Content-length: " . strlen($tb_send);

	// Send the trackback
	fputs($tb_sock, $tb_headers);
	fputs($tb_sock, "\r\n\r\n");
	fputs($tb_sock, $tb_send);
	// Gather result
	while (!feof($tb_sock)) {
	...
	// Did the trackback ping work
	strpos($response, '<fault>') ? $return = false : $return = true;
	
	$query = "INSERT INTO `out_trackback` (`target`, `headers`, `send`, `response`) VALUES ('" .
		mysql_real_escape_string($tb) . "', '" .
		mysql_real_escape_string($tb_headers) . "', '" .
		mysql_real_escape_string($tb_send) . "', '" .
